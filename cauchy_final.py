...
for i in range(0,n):
    random.seed()
    x=stats.cauchy.rvs(881.5,0.47,19)
    ...
    prob = [((2**-N)*factorial(N))/(factorial(q)*factorial(N-q)) for q in range(1, N+1) ]
    #prob1 = prob[0:23]
    df_data["prob"] = prob
    ...
    D_norm, ks_arr[i,0] = stats.kstest(data_arr[:,0], 'cauchy')
    
    ###p-values forcale factor=1 for all distributions(weighted mean-)
    D_norm, ks_arr[i,1] = stats.kstest(data_arr[:,1], 'cauchy')
    
    ###p-values for cale factor=1 for all distributions(arithmetic mean)
    #D_norm, ks_arr[i,2] = stats.ks_2samp(binned_am, z_am_c)
    
    ###p-values for scale factor=1 for all distributions(median)
    D_norm, ks_arr[i,2] = stats.kstest(data_arr[:,2], 'cauchy')
    
    
    D_norm, ks_arr_scale[i,0] = stats.kstest(data_arr[:,0], 'cauchy', args =(0,0.817))
    D_norm, ks_arr_scale[i,1] = stats.kstest(data_arr[:,1], 'cauchy', args =(0,0.85))
    #D_norm, ks_arr_scale[i,2] = stats.ks_2samp(binned_am, stats.cauchy.pdf(x = center_am, scale =s))
    D_norm, ks_arr_scale[i,2] = stats.kstest(data_arr[:,2], 'cauchy', args =(0,0.786))
